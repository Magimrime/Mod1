package net.oliver.forgemod.client.render.model;
...
import net.oliver.forgemod.config.Config;

public class WalnutVillagerModel<T extends AbstractVillager> extends VillagerModel<T> implements ArmedModel {
    private static final ResourceLocation MODEL_ID = ResourceLocation.fromNamespaceAndPath(ForgeMod.MOD_ID, "walnut_villager");
    public static final ModelLayerLocation MAIN_LAYER_LOCATION = new ModelLayerLocation(MODEL_ID, "main");
    public static final ModelLayerLocation OVERLAY_LAYER_LOCATION = new ModelLayerLocation(MODEL_ID, "overlay");
    ...
    public WalnutVillagerModel(ModelPart root) {
        super(root);
    ...
    public void copyPropertiesTo(WalnutVillagerModel<T> model) {
    ...
    public void renderToBuffer(PoseStack poseStack, VertexConsumer buffer, int packedLight, int noOverlay, float v, float v1, float v2, float v3) {
    }
}